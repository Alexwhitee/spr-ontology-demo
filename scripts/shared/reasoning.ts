import type { CurveSummary, ProcessRecord, ReasoningResult } from "./types";

export function summarizeCurve(points: number[]): CurveSummary {
  if (points.length === 0) {
    return { pointCount: 0, min: 0, max: 0, avg: 0, peakIndex: -1 };
  }
...
export function parseCurve(value: unknown): number[] {
